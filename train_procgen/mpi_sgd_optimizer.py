...
class MpiSGDOptimizer(tfa.optimizers.AdamW):
    """Adam optimizer that averages gradients across mpi processes."""
    def __init__(self, comm, grad_clip=None, mpi_rank_weight=1, **kwargs):
        self.comm = comm
        self.grad_clip = grad_clip
        self.mpi_rank_weight = mpi_rank_weight
        tf.keras.optimizers.SGD.__init__(self, **kwargs)
    def compute_gradients(self, loss, var_list, **kwargs):
        grads_and_vars = tf.keras.optimizers.SGD.compute_gradients(self, loss, var_list, **kwargs)
...
def test_nonfreeze():
    ...
    update_op = MpiSGDOptimizer(comm=MPI.COMM_WORLD, learning_rate=stepsize).minimize(loss)
